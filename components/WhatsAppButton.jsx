...
const WhatsAppButton = ({ student, onMessageSent, lesson, isStudentMessage = false }) => {
  ...
  const handleWhatsAppClick = () => {
    ...
    try {
      ...
      const originalPhoneNumber = isStudentMessage 
        ? student.phone 
        : student.parents_phone;
      
      // Check if phone number starts with "+" (not allowed)
      if (originalPhoneNumber && originalPhoneNumber.trim().startsWith('+')) {
        setMessage('Invalid phone number: cannot start with "+"');
      ...
      // Remove non-numeric characters to get clean phone number
      let phoneNumber = originalPhoneNumber ? originalPhoneNumber.replace(/[^0-9]/g, '') : null;
      ...
      // Format phone numbers: if starts with "0", use default country code (20)
      // Otherwise, use the number as-is (already has country code)
      if (phoneNumber.startsWith('0')) {
        phoneNumber = '20' + phoneNumber.substring(1);
      }
      // If it doesn't start with "0", use it as-is (e.g., 971501234567)

...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (videos && Array.isArray(videos) && videos.length > 0) {
        ...
        for (let index = 0; index < videos.length; index++) {
          ...
          if (video && video.video_type && video.video_id) {
            ...
            } else if (video.video_type === 'vdocipher') {
              videoData[`video_ID_${index + 1}`] = video.video_id;
              videoData[`video_type_${index + 1}`] = 'vdocipher';
            } else {
              return res.status(400).json({ error: `Invalid video type at position ${index + 1}. Must be "youtube" or "vdocipher"` });
            }
          } else {
      ...
      // Remove old video_ID and video_type fields that are not in the new list
      const keysToRemove = Object.keys(session).filter(key => 
        (key.startsWith('video_ID_') || key.startsWith('video_type_')) && 
        !videoData[key]
      );

...
export default function AddOnlineSession() {
  ...
  const [formData, setFormData] = useState({
    ...
    description: '',
    videos: [{
      video_source: 'youtube', // 'youtube' or 'vdocipher'
      youtube_url: '',
      vdocipher_option: 'video_id', // 'video_id' or 'upload'
      vdocipher_video_id: '',
      vdocipher_file: null,
      vdocipher_uploaded_video_id: null // Store uploaded video ID
    }]
  });
  ...
  const handleVdoCipherFileChange = async (index, file) => {
    ...
    try {
      // Convert file to base64 with progress tracking (0-20%)
      ...
      // Start upload progress (20-98%)
      startTime = Date.now();
      let uploadProgress = 20;
      setUploadingVideos(prev => ({ ...prev, [index]: uploadProgress }));
      
      // Smooth progress increment based on elapsed time
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate each video
    // Validate each video and check VdoCipher IDs exist
        ...
        <div className="form-container" style={{
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              {formData.videos.map((video, index) => (
                <div key={index} style={{
                  ...
                  {/* Video Source Radio (YouTube or VdoCipher) */}
                  ...
                  {video.video_source === 'vdocipher' && (
                    <>
                      {/* VdoCipher Sub-Radio (Video ID or Upload) */}
                      <div style={{ marginBottom: '16px' }}>
                        <label style={{ display: 'block', marginBottom: '8px', fontWeight: '600' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
              <button
                type="submit"
                disabled={createSessionMutation.isPending || Object.values(uploadingVideos).some(v => v !== undefined && v < 100)}
                style={{
                  padding: '12px 24px',
                  backgroundColor: (createSessionMutation.isPending || Object.values(uploadingVideos).some(v => v !== undefined && v < 100)) ? '#6c757d' : '#28a745',
                  color: 'white',
                  border: 'none',
                  borderRadius: '6px',
                  fontSize: '1rem',
                  fontWeight: '500',
                  cursor: (createSessionMutation.isPending || Object.values(uploadingVideos).some(v => v !== undefined && v < 100)) ? 'not-allowed' : 'pointer',
                  opacity: (createSessionMutation.isPending || Object.values(uploadingVideos).some(v => v !== undefined && v < 100)) ? 0.6 : 1
                }}
              >
                {createSessionMutation.isPending ? 'Creating...' : Object.values(uploadingVideos).some(v => v !== undefined && v < 100) ? 'Uploading...' : 'Create Session'}
              </button>
              <button
                type="button"

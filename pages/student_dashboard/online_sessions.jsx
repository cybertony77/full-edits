...
// VdoCipher Player Component
function VdoCipherPlayer({ videoId }) {
  ...
  useEffect(() => {
    const fetchVdoCipherData = async () => {
      try {
        setLoading(true);
        const response = await apiClient.post('/api/vdocipher/get-otp', { video_id: videoId });
        if (response.data.success && response.data.otp && response.data.playbackInfo) {
          setOtp(response.data.otp);
          setPlaybackInfo(response.data.playbackInfo);
        } else {
          setError('Failed to load VdoCipher video');
        }
      } catch (err) {
  ...
  return (
    <iframe
      src={`https://player.vdocipher.com/v2/?otp=${otp}&playbackInfo=${encodeURIComponent(playbackInfo)}`}
      frameBorder="0"
...
export default function OnlineSessions() {
      ...
      if (response.data.success && response.data.valid) {
        ...
        if (selectedSession) {
          ...
          if (selectedSession.video_ID) {
            videoId = selectedSession.video_ID;
            videoType = selectedSession.video_type || 'youtube';
          } else {
            while (selectedSession[`video_ID_${videoIndex}`]) {
              videoId = selectedSession[`video_ID_${videoIndex}`];
              videoType = selectedSession[`video_type_${videoIndex}`] || 'youtube';
              break;
            }
          }
          
          if (videoId) {
            setSelectedVideo({ ...selectedSession, video_ID: videoId, video_type: videoType });
  ...
  const openVideoPopup = async (session, videoId, videoIndex) => {
    ...
    if (session.payment_state === 'free') {
      ...
      setSelectedVideo({ ...session, video_ID: videoId, video_type: videoType });
      setVideoPopupOpen(true);
      videoStartTimeRef.current = Date.now();
      return;
    }
          ...
          >
            ...
            >
              ...
              <div 
                ...
                onSelectStart={(e) => e.preventDefault()}
              >
                {selectedVideo.video_type === 'vdocipher' ? (
                  <VdoCipherPlayer videoId={selectedVideo.video_ID} />
                ) : (
                  <iframe
